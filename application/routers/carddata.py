...
def get_user_collection(db : Session = Depends(database.get_db), current_user = Depends(oauth2.get_current_user)):
    ...
    if(cards_details==None):
        raise HTTPException(
            status_code =  status.HTTP_204_NO_CONTENT,
            detail = "Your collection us empty"
        )
    card_data = []
    for cards in cards_details:
        card_obj, card_count = cards
        data = schemas.Card.model_validate(card_obj).model_dump()
        data["card_count"] = card_count
        card_data.append(data)
